...
import invetoryFakerFactory from "./invetoryFakerFactory";
...
const InvetoryFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = invetoryFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("invetory").create(elem));
...
export default connect(null, mapDispatch)(InvetoryFakerDialogComponent);
