...
import storesFakerFactory from "./storesFakerFactory";
...
const StoresFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = storesFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("stores").create(elem));
...
export default connect(null, mapDispatch)(StoresFakerDialogComponent);
