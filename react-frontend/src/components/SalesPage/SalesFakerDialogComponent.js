...
import salesFakerFactory from "./salesFakerFactory";
...
const SalesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = salesFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("sales").create(elem));
...
export default connect(null, mapDispatch)(SalesFakerDialogComponent);
